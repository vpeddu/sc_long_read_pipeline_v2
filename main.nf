...
workflow {
    ...
    // Find all fastq files in input directory and pair with barcodes
    def input_path = file(params.input_dir)
    if (!input_path.isDirectory()) {
        error("--input_dir must be a directory, got: ${params.input_dir}")
    }
    ...
    sample_data = Channel
        ...
        .map { fastq_file ->
            ...
            def barcode_file = file("${params.input_dir}/*.barcodes.tsv")
    ...
    A02_minimap = runMinimap2(flex_output,
        ref_junc_bed,
        ref_genome_fa,
        file("${projectDir}/original_scripts/bam_tag_bc_umi.py")
        )
    A03_dedup = runDeDup(A02_minimap,
        file("${projectDir}/original_scripts/collapse_barcodes.py"),
        ref_genes_gtf
        )

...
- Current container: `ghcr.io/vpeddu/sc_long_pipeline:v1.4.0`, which added an `orfpred` conda env (`td2` + `psauron` + `hmmer`) for `A08_orf.nf`'s ORF/domain prediction, on top of `v1.3.1`'s `salmon` conda env (`salmon` 1.10.2 + `alevin-fry`, re-pinned after salmon 2.x dropped the `alevin` subcommand entirely) for `D01_shortread.nf`, `v1.2.0`'s `seurat` conda env (Seurat + tximport, R 4.3) for `A06_seurat.nf`, and the existing `long_reads`/`flair`/`vep`/`picardtools`/`isoSeQL`/`bioconductor`/`sqanti3` environments.
...
- `A08_orf.nf` (`--predict_orfs`) uses `td2`/`psauron`/`hmmer`, also in their own dedicated conda env — kept fully separate from the isoSeQL-pinned SQANTI3 branch (see below).
...
- `A08_orf.nf`: ORF prediction and Pfam domain classification against each sample's novel transcriptome — see `--predict_orfs` below.
...
- `--exclude_chrm` (`true`/`false`, default `true`): mitochondrial read depth is vastly disproportionate to chrM's size, making flair transcript calling on it a major runtime outlier. When `true`, `--flair_split_by_chrom`'s per-chromosome path skips chrM entirely (no `runFlairByChrom` task is ever created for it — real time savings, not just output filtering), and `runTxRename` drops any chrM transcripts from the final GTF/xref/isoform-matrix/txmod-fasta output regardless of which flair mode ran, so behavior stays consistent either way. Set `false` to keep chrM isoforms (accepting the runtime cost). Since chrM is never passed through flair in this mode, `--keep_intergenic true`'s `A05_htseq` quantification (which otherwise runs against the flair-derived gtf instead of the reference) would silently lose chrM gene counts entirely — `runChrmGeneSubset`/`mergeChrmReferenceGenes` backfill chrM's genes straight from the reference annotation and merge them into that per-sample gtf before htseq runs, so chrM gene counts are always present regardless of `--exclude_chrm`/`--keep_intergenic`.
...
- `--predict_orfs` (`true`/`false`, default `true`): predicts ORFs and classifies Pfam domains for each sample's own novel transcriptome, using [TD2](https://github.com/Markusjsommer/TD2) (a modern, MIT-licensed TransDecoder successor) + its internal [PSAURON](https://github.com/salzberg-lab/PSAURON) coding-likelihood scorer + `hmmsearch` against Pfam-A. This is fully independent of `C01_SQANTI3.nf`/`C02_isoSeQL.nf`: that branch runs an old SQANTI3 fork pinned specifically for isoSeQL compatibility, and its own ORF prediction (`coding`/`ORF_length`/etc. columns, currently always empty) needs the license-gated GeneMarkS-T, which can't be bundled into a shared container — so ORF prediction is done here instead, with its own tools and its own Seurat metadata columns, and never touches the SQANTI3/isoSeQL pipeline. Standard TransDecoder-style workflow: `TD2.LongOrfs` extracts candidate ORFs from the per-sample renamed transcriptome FASTA, `hmmsearch` (with `--cut_ga`, Pfam's own gathering thresholds) scores them against Pfam-A, and `TD2.Predict --retain-hmmer_hits` makes the final call — any candidate with a Pfam domain hit is retained even if its own PSAURON score alone wouldn't have passed. Pfam-A.hmm (~400MB compressed) is downloaded and `hmmpress`-indexed once per run (cached across `-resume`d runs the same way the STAR genome index already is), not baked into the container.

`--flair_split_by_chrom`, `--keep_intergenic`, `--exclude_chrm`, and `--predict_orfs` accept the literal strings `true`/`false` on the command line (e.g. `--keep_intergenic true`); pass them explicitly rather than as bare flags.
...
- `A06_seurat`: one `<sample>.seurat.rds` Seurat object per sample, with an `RNA` assay (gene-level counts) and an `ISO` assay (isoform-level counts). Cells are tagged with `orig.ident` and barcode-prefixed by sample so multiple samples' objects can be merged safely. Feature-level metadata includes genomic ranges (`chrom`/`start`/`end`/`strand`, both assays) and, on the `ISO` assay, SQANTI3 classification columns (`structural_category`, `associated_gene`, `length`, `exons`, coverage/coding/NMD/filter columns, plus `perc_A_downstream_TTS`/`RTS_stage` for spot-checking internal-priming/RT-switching artifacts — `filter_result` alone only gives the final Artifact/Isoform verdict, not which rule triggered it) and `read_support` (flair's own supporting-read count for that transcript, from `transcript_xref.tsv`'s `score` column — lost if you only look at SQANTI3's corrected gtf, which hardcodes score as `.`). Per-cell metadata includes `raw_reads`, `predup_reads`, `postdup_reads`, and `dedup_rate` (also available as a compact list in `@misc$sample_metrics`).
...
- `A08_orf` (only produced when `--predict_orfs true`, the default): `<sample>.TD2.{pep,cds,gff3,bed}` (TD2's final predicted ORFs — protein, CDS nucleotide, and genomic/transcript-coordinate annotations) and `<sample>.pfam.domtblout` (the raw `hmmsearch` Pfam domain hits used to produce them). The same information is merged into the `A06_seurat` object's `ISO` assay metadata as `orf_type` (`complete`/`5prime_partial`/etc.), `orf_len` (predicted protein length, aa), `psauron_score`, `pfam_domain_count`, `pfam_best_domain`, and `pfam_best_evalue` — `NA` for transcripts with no predicted ORF (or no domain hit). These are distinct from (and, unlike) the SQANTI3-native `coding`/`ORF_length`/etc. columns already on the `ISO` assay, which are currently always empty/`non_coding` since that branch's own ORF prediction is disabled (see `--predict_orfs` above).

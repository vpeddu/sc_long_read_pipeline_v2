...
`--flair_split_by_chrom`, `--keep_intergenic`, and `--exclude_chrm` accept the literal strings `true`/`false` on the command line (e.g. `--keep_intergenic true`); pass them explicitly rather than as bare flags.
